...
Hard.prototype = {
	...
	moveEnemy: function(){
		diffx = Math.abs(enemy.position.x - player.position.x);
		diffy = Math.abs(enemy.position.y - player.position.y);
		if(diffx > diffy){ 
			if (player.position.x < enemy.position.x) {
				enemy.position.x -= pwidth;
	...
	ateFood: function() {
		...
		game.time.events.loop(loopTimer, Hard.prototype.movePlayer, this);
		game.time.events.loop(loopTimer+240, Hard.prototype.moveEnemy, this);
	
		scoreText.text = "SCORE: " + score;
		tickCount = 0;
		old_pos = JSON.stringify(food.position);
		
		Hard.prototype.randFood();
		return;
	},
	...
	create: function(){
		...
		//create enemy
		enemy = game.add.sprite(20,80,'enemy');
		...
		game.time.events.loop(loopTimer+240, this.moveEnemy, this);
